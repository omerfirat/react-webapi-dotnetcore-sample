...
export default function XVpnSmsList() {
    const [rows, setRows] = React.useState([]);

    const [isLoading, setLoading] = React.useState(false);
    const [selectedBeginingDate, setSelectedBeginingDate] = React.useState(new Date('2020-05-12T21:11:54'));
    const [selectedEndingDate, setSelectedEndingDate] = React.useState(new Date('2020-05-26T21:11:54'));
    const [selectedPersonal, setSelectedPersonal] = React.useState('');
    ...
    const showError = (event) => {
        ...
        fetch('api/request/errorByCode/' + event.target.textContent, requestOptions)
    ...
    const handleSubmit = (event) => {
        ...
        setLoading(true);
        fetch('api/request/vpn_list/list_all', requestOptions)
            .then(function (response) {
                return response.json();
            })
            .then(function (result) {
    ...
    const onRowStatusRequest = (event, rowdata) => {
        ...
        fetch('api/request/message_by_id/' + rowdata.id, requestOptions)
            .then(function (response) {
                return response.json();
            })
            .then(function (res) {
                let result = rows.map((rows, id) => {
                    if (rows.id === rowdata.id) {
                        return { ...rows, status: res.status,errorCode:res.errorCode };
                    }
    ...
    if (getLogin()) {
        ...
        return (
            <div>
                {/*h1 id="tabelLabel" >Fast Sms List</h1>*/}
                <p>This component demonstrates fetching data from the Vpn Login Sms</p>
                ...
                <form className={classesSearch.container} onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <FormControl className={classesFormSelect.formControl}>
                        ...
                        >
                            ...
                            <MenuItem value={"3"}>Not Found</MenuItem>
                        </Select>
